...
public:
    Resol(UARTComponent *parent) : PollingComponent(30000),
                                   UARTDevice(parent) {};

    Sensor *collector_temp_sensor = new Sensor();
    Sensor *upper_tank_temp_sensor = new Sensor();
    Sensor *lower_tank_temp_sensor = new Sensor();
    Sensor *cv_return_temp_sensor = new Sensor();
    Sensor *solarpump_speed_sensor = new Sensor();
    Sensor *cv_valve_state_sensor = new Sensor();

    float get_setup_priority() const override { return esphome::setup_priority::AFTER_WIFI; }

    bool isValid(float temp) {
        return (temp > -20.0 && temp < 150.0);
    }
    ...
    void update() override {
        ...
        float collTemp = getS1Temp();
        float topTemp = getS3Temp();
        float btmTemp = getS2Temp();
        float cvRetTemp = getS4Temp();
        int pumpSpeed = getP1Speed();
        int valveState = getP2Status();

        if (isValid(collTemp) && isValid(topTemp) && isValid(btmTemp)) {
            collector_temp_sensor->publish_state(collTemp);
            upper_tank_temp_sensor->publish_state(topTemp);
            lower_tank_temp_sensor->publish_state(btmTemp);
            cv_return_temp_sensor->publish_state(cvRetTemp);
            solarpump_speed_sensor->publish_state(pumpSpeed);
            cv_valve_state_sensor->publish_state(valveState);
        } else {
            ESP_LOGE("Resol::update()", "Error, invalid temps coll:%02f, top:%02f, btm:%02f",
                     collTemp, topTemp, btmTemp);
        }
    }
    ...
    bool readSensor() {
        ...
        */
        ...
        if (SystemNotification != 0x00 ||
            ErrorMask != 0x00) // Not really sure what ErrorMask is, treating as system alert.
        {
            SystemAlert = true;
        } else {
            SystemAlert = false;
...
private:
    ...
    bool vBusRead() {
        ...
        if (!quit) {
            ...
            if ((Command == 0x0100) and (Bufferlength == 10 + Framecnt * 6) and
                ...
                if (Source_address == 0x3271) {
                    ...
                    if (Buffer[F + 5] == VBus_CalcCrc(Buffer, F, 5)) { // CRC ok
                        ...
                        InjectSeptet(Buffer, F, 4);
                        HeatQuantity =
                                Buffer[F] + Buffer[F + 1] * 256 + Buffer[F + 2] * 65536 + Buffer[F + 3] * 16777216;
                    }
                    ///******************* End of frames ****************
